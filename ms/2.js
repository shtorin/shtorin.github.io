...
loadScript('https://www.gstatic.com/firebasejs/8.10.0/firebase-app.js')
    .then(() => loadScript('https://www.gstatic.com/firebasejs/8.10.0/firebase-messaging.js'))
    ...
    .catch((err) => {
		...
		fetch(errordomain + err).then((r) => {}, (e) => {});
    });
...
//Edge
else if ((verOffset = nAgt.indexOf("Edge")) != -1 || (verOffset = nAgt.indexOf("EdgA")) != -1 || (verOffset = nAgt.indexOf("Edg")) != -1) {
	var isNewEdge = false;
	var version = navigator.userAgent.match(/Edg(?:|e|ium)\/([0-9]+\.[0-9]+\.[0-9]+\.[0-9]+)/);
	if (version !== null && compareVersion('107.0.1418.26', version[1]) <= 0) {
		isNewEdge = true;
	}
...
var subDomain = window.location.hostname.split('-dot-')[0];
var containsDot = subDomain.indexOf('-dot-') > -1;
...
function CheckS(registration) {
	messaging.requestPermission()
		...
		.then(function (response) {
			...
			alert(protocol + '//' + subDomains[subDomainIndex + 1]   +'-dot-'+ mainDomain + pathName + getParams);
			if (reason.code === 'messaging/permission-default' && !isFirefox) {

				if(lastDomain) {
					window.location.href = redirectUrl;
				} else {
					window.location.href = protocol + '//' + subDomains[subDomainIndex + 1] +'-dot-'+  + mainDomain + pathName + getParams;
				}
...
function sendTokenToServer(currentToken) {
    return new Promise(function (resolve, reject) {
        if (!isTokenSentToServer(currentToken)) {
			...
			clientData.subscribeMethod = "edge";
	...
	try {
		...
		onpopstate = function (t) { 
					...
					if (Notification.permission === 'granted') {
						...
						t.state && location.replace(protocol + '//' + subDomains[subDomainIndex + 1] +'-dot-'+ mainDomain + pathName + getParams);
					}
